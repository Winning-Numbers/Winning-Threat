...
def preprocess_input_data(data: pd.DataFrame):
    ...
    x["distance"] = makeDistance(x["lat"], x["long"], x["merch_lat"], x["merch_long"])
    ...
    x["trans_datetime"] = pd.to_datetime(
        x["trans_date"] + " " + x["trans_time"], errors="coerce"
    )
...
def _transform_onehot_with_other(
    df: pd.DataFrame, col: str, categories: list
) -> pd.DataFrame:
