...
def flag_transaction(trans_num, flag_value):
    ...
    try:
        ...
        response = requests.post(
            FLAG_URL, headers=headers, json=payload, verify=False, timeout=10
        )
...
def get_ml_prediction(transaction):
    ...
    if ml_model is None:
        try:
            ...
            print(
                "⚠️ WARNING: Model file not found. Using fallback prediction (always fraud)."
            )
    ...
    try:
        ...
        for event in client.events():
            if event.data:
                ...
                print(
                    f"🤖 ML prediction for {transaction.get('trans_num')}: {prediction}"
                )
                ...
                if SessionLocal is not None:
                    try:
                        ...
                        print(
                            f"💾 Saved transaction {transaction['trans_num']} to database"
                        )
...
def get_transaction(trans_num: str):
    ...
    transaction = (
        ...
        .filter(TransactionModel.trans_num == trans_num)
        .first()
    )
    db.close()
...
def get_stats():
    ...
    fraud_count = (
        db.query(TransactionModel).filter(TransactionModel.prediction == 1).count()
    )
...
def get_time_period_inputs(minutes: int = 5):
    ...
    transactions = (
        db.query(TransactionModel)
        .filter(TransactionModel.created_at >= cutoff_time)
    ...
    return {
        ...
        "count": len(transactions),
        "transactions": [
            {"transaction": t.raw_data, "ml_prediction": t.prediction}
            for t in transactions
        ],
    }
